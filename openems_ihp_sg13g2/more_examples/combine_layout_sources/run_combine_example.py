...
# Model comments
# 
# This is to demonstrate how multiple ways to create a geometry can be combined.
# However, the automatic mesh is only created from the polygons from the GDSII file and polygons added to the allpolygons data structure.
# There is no automatic mesh for geometries created by standard openEMS commands!
...
# ===> ADD SOME MORE POLYGONS AND BOXES USING OUR INTERNAL DATA STRUCTURE  <===
# These will be added to the allpolygons data structure and included in automatic meshing!

# ground plane on Metal1 
allpolygons.add_rectangle(x1=100,  y1=-50, x2=150, y2=0, layernum=metals_list.getbylayername('Metal5').layernum)

# test adding polygon 
allpolygons.add_polygon( xy=[[-20,-20],[-20,20],[120,20],[120,-20]], layernum=metals_list.getbylayername('Metal3').layernum)
...
simulation_setup.setupSimulation (excite_ports, 
                                    simulation_ports, 
                                    FDTD, 
                                    materials_list, 
                                    dielectrics_list, 
...
# ===> ADD SOME MORE SHAPES USING STANDARD OPENEMS API <===
# These are NOT included in automatic meshing, you need to add mesh lines manually in this code!
...
start = [250, -50, 250]
stop  = [300,  50, 260]
# We need to know priorities of dielectric that is added by setupSimulation():
# Metals are priority 200, dielectrics are priority 10
# To place a PEC inside the dielectic created by setupSimulation() we use priority 210 now:
patch.AddBox(priority=210, start=start, stop=stop) 
FDTD.AddEdges2Grid(dirs='xyz', properties=patch, metal_edge_res=refined_cellsize)


# ===> ADD SOME MORE SHAPES FROM STL <===
STL1 = CSX.AddMetal('Metal')
STL1.AddPolyhedronReader(os.path.join(os.path.dirname(__file__),'example.stl'), priority=220).ReadFile()

# we would need to add sufficient mesh now, this is not automatic!
# mesh.AddLine('z', [300.0, 305.0] )
